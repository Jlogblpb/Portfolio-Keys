package com.jlogblpb.myapplication
...
class MainActivity : AppCompatActivity() {
    ...
    private var numberHouse: EditText? = null
    private var korpus: EditText? = null
    private var keyResults: TextView? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
       ...
       val streets = arrayOf( "героев сталинграда", "пролетарская", "бульвар энгельса",
        ...
        street = findViewById <AutoCompleteTextView> (R.id.autoCompleteStreet)

        //Пишем адаптер
        val adapter = ArrayAdapter(this,android.R.layout.simple_list_item_1, streets)
        (street as AutoCompleteTextView?)?.setAdapter(adapter)
    ...
    fun funFindKey() //Функция выполняевамя по нажатию кнопки "Поиск"
    {
        //Массив кампусов
        ...
        //Считыаем переменные из полей
        numberHouse = findViewById(R.id.NumberHouse)
        korpus = findViewById(R.id.Korpus)
        keyResults = findViewById(R.id.KeyResults)


        //Индексы для поиска по массиву
        var IndexStreet: Int = -1  //индекс найденной улицы
        var IndexHome: Int  // Индекс номера дома
        var IndexKorpus: Int //ИндексКорпуса (0-4)

            //Поиск индекса для улицы
        for (i in streets.indices)
        {
            if (street?.text.toString() == streets[i])
            {
                IndexStreet = i
                break
            }
            ...
            {
                keyResults?.setText("Искомая улица " + street?.text.toString() +" не найдена. " +
                        "Проверьте правильность написания!" )
            }
        }

             //Поиск Корпуса (0-4)
        if (korpus?.text.toString() == "а")
            {IndexKorpus = 1}
        else if (korpus?.text.toString() == "б")
            {IndexKorpus = 2}
        else if (korpus?.text.toString() == "в")
            {IndexKorpus = 3}
        else if (korpus?.text.toString() == "г")
            {IndexKorpus = 4}
        else if (korpus?.text.toString() == "д")
            {IndexKorpus = 5}
        else
        {IndexKorpus = 0}
        IndexHome = numberHouse?.text.toString().toInt()

        //Поиск в основном массиве
        if (IndexStreet > -1) {
            for (i in key.indices) {
                if (IndexStreet == key[i][0])
                {
                    if (IndexHome == key[i][1])
                    {
                        if (IndexKorpus == key[i][2])
                        {
                            keyResults?.setText("Искомый ключ № " + keys[key[i][3]].toString() +
                                        ". Кампус " + kampus[key[i][4]].toString() + "! " +
                                    comments[key[i][5]].toString())
                            break
                        }
                    }
                }
                ...
                {
                    keyResults?.setText("Ничего не найдено!!!  Попробуйте изменить вводимые данные!")
                }
            }
        }
    }
